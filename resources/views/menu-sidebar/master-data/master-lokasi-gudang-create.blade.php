...
@section('title', 'Tambah Lokasi Gudang')
    ...
    <div class="page-title">
        <div class="row">
            <div class="col-12 col-md-6 order-md-1 order-last">
                ...
                <p class="text-subtitle text-muted">Tambah lokasi penyimpanan barang NG baru</p>
            </div>
    ...
    <section class="section">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title">Form Tambah Lokasi Gudang</h4>
            </div>
            <div class="card-body">
                <form action="{{ route('master-lokasi-gudang.store') }}" method="POST">
                    @csrf
                        ...
                        <div class="col-md-4">
                            <div class="form-group mb-3">
                                ...
                                <select class="form-select @error('zone') is-invalid @enderror" id="zone" name="zone" required>
                                    ...
                                    <option value="zona_a" {{ old('zone') == 'zona_a' ? 'selected' : '' }}>Zona A</option>
                                    <option value="zona_b" {{ old('zone') == 'zona_b' ? 'selected' : '' }}>Zona B</option>
                                    <option value="zona_c" {{ old('zone') == 'zona_c' ? 'selected' : '' }}>Zona C</option>
                                    <option value="zona_d" {{ old('zone') == 'zona_d' ? 'selected' : '' }}>Zona D</option>
                                </select>
                                @error('zone')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-md-6">
                            <div class="form-group mb-3">
                                ...
                                <input type="number" class="form-control @error('kapasitas_max') is-invalid @enderror" 
                                       ...
                                       value="{{ old('kapasitas_max') }}" required>
                                @error('kapasitas_max')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-12">
                            <div class="form-group mb-3">
                                ...
                                <textarea class="form-control @error('deskripsi') is-invalid @enderror" 
                                          ...
                                          placeholder="Deskripsi atau catatan lokasi gudang">{{ old('deskripsi') }}</textarea>
                                @error('deskripsi')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
...
$(document).ready(function() {
    ...
    // Notify parent window when lokasi is created (if opened in popup)
    @if(session('close_popup'))
        if (window.opener) {
            localStorage.setItem('new_lokasi_added', Date.now());
            window.close();
        }
    @endif
});
